...
sequential_env_list = ['cartpole-balance', 'cartpole-swingup']
def sequential_env(sequential_env_list):
  """Chain together a list of environments to create a sequential environment.""" 
  for env in sequential_env_list:
      env = Env(env, symbolic=False, seed=0, max_episode_length=1000, action_repeat=1, bit_depth=8)
      obs = env.reset()
      for _ in range(100):
          env.render()
          env.step(env.sample_random_action())
      env.close()
sequential_env(sequential_env_list)

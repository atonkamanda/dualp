...
class RSSMPosterior(nn.Module):
    ...
    def __init__(self, hidden_dim=200, state_dim=30, scale_lb=0.1):
        super().__init__()
        self.obs_rnn_to_post_projector = NormalParamWrapper( # NormalParamWrapper is a wrapper for that modifies the output of nn that output mean and std (e.g fix the std to be positive)
            nn.Sequential(
...
posterior = RSSMPosterior(hidden_dim=200, state_dim=30, scale_lb=0.1)
belief = torch.randn(1, 200)
state = torch.randn(1, 30)
action = torch.randn(1, 4)
obs_embedding = torch.randn(1, 200)
prior_mean, prior_std, state, belief = prior(state, belief, action)
posterior_mean, posterior_std, state = posterior(belief, obs_embedding)
#print(prior_mean.shape, prior_std.shape, state.shape, belief.shape)
#print(posterior_mean.shape, posterior_std.shape, state.shape)
...
rssm_rollout = RSSMRollout(prior, posterior)
rollout_result = rssm_rollout(tensordict)